...
def upgrade() -> None:
    ...
    eventstatus = postgresql.ENUM('not_started', 'in_process', 'finished', name='eventstatus')
    eventstatus.create(op.get_bind())
    op.add_column('events',
                  sa.Column('status', postgresql.ENUM('not_started', 'in_process', 'finished', name='eventstatus',
                                                      create_type=False)))
...
def downgrade() -> None:
    ...
    op.alter_column('events', 'start_time',
                    existing_type=postgresql.TIMESTAMP(timezone=True),
                    nullable=True)
    eventstatus = postgresql.ENUM('active', 'inactive', 'archive', name='banner_status')
    eventstatus.drop(op.get_bind())
